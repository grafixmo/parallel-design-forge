 ...
 */
export const importSVG = async (svgContent: string, options?: {
  onProgress?: (progress: number) => void,
  maxObjects?: number
}): Promise<BezierObject[]> => {
  const { onProgress, maxObjects } = options || {};
  try {
    console.log('Starting SVG import with Fabric.js');
    onProgress?.(10);
    
    // Use the improved parser
    const result = await parseSVGWithFabric(svgContent, onProgress);
    
    onProgress?.(100);
    console.log(`Successfully imported ${result.length} objects`);
    
    // Apply object limit if specified
    if (maxObjects && result.length > maxObjects) {
      console.warn(`Limiting imported objects from ${result.length} to ${maxObjects}`);
...
/**
 * Parse SVG content using Fabric.js with improved error handling and performance
 */
const parseSVGWithFabric = async (
  svgContent: string, 
  onProgress?: (progress: number) => void
): Promise<BezierObject[]> => {
  ...
  // Load the SVG content with proper async handling
  return new Promise<BezierObject[]>((resolve, reject) => {
    try {
      // Using Fabric.js to load SVG
      FabricCanvas.prototype.loadSVGFromString(svgContent, async (fabricObjects: FabricObject[], options: any) => {
        ...
        // Process objects in chunks to prevent UI freezing
        const chunkSize = 10;
        const chunks = Math.ceil(fabricObjects.length / chunkSize);
        
        for (let i = 0; i < chunks; i++) {
          const start = i * chunkSize;
          const end = Math.min(start + chunkSize, fabricObjects.length);
          const chunk = fabricObjects.slice(start, end);
          
          // Process each chunk
          await new Promise<void>(resolve => {
            setTimeout(() => {
              chunk.forEach((fabricObject, index) => {
                if (fabricObject.type === 'path') {
                  try {
                    const pathObj = fabricObject as unknown as Path;
                    const bezierObject = convertFabricPathToBezierObject(pathObj, start + index);
                    if (bezierObject) {
              ...
              // Update progress
              const progress = 40 + Math.floor((i + 1) / chunks * 40);
              onProgress?.(progress);
      ...
      console.error('Error in parseSVGWithFabric:', error);
      canvas.dispose();
      reject(error);
    }
  });
